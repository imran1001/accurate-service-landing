...
function Index() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans antialiased">
      ...
      <div ref={paymentRef}>
        {submitted && (
          <section className="border-y border-gold/30 bg-navy text-navy-foreground">
            <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8">
              <div className="text-center">
                <span className="inline-block rounded-full bg-gold/20 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-gold">
                  Step 2 of 2
                </span>
                <h2 className="mt-4 text-3xl font-bold">Complete Your Payment</h2>
              ...
              <div className="mt-10 grid gap-5 sm:grid-cols-2">
                <div className="rounded-2xl border border-gold/30 bg-navy-foreground/5 p-6">
                  <div className="text-xs font-semibold uppercase tracking-widest text-gold">Bank Transfer</div>
                  <div className="mt-4 space-y-2 text-sm">
                    <Row label="Account Title" value="Muhammad Imran Malik" />
                    <Row label="Bank" value="MCB Bank Limited, Lahore" />
                    <Row label="Account #" value="1069209731000337" mono />
                    <Row label="IBAN" value="PK85MUCB1069209731000337" mono />
                  </div>
                </div>
                <div className="rounded-2xl border border-gold/30 bg-navy-foreground/5 p-6">
                  <div className="text-xs font-semibold uppercase tracking-widest text-gold">EasyPaisa</div>
                  <div className="mt-4 space-y-2 text-sm">
                    <Row label="Account Title" value="Muhammad Imran Malik" />
                    <Row label="Number" value="0316 0285386" mono />
                  </div>
                </div>
              </div>

              <div className="mt-8 rounded-xl border-l-4 border-gold bg-gold/10 p-5 text-sm font-semibold leading-relaxed text-navy-foreground">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-6 flex w-full items-center justify-center gap-3 rounded-xl bg-[oklch(0.68_0.17_150)] py-4 text-base font-semibold text-white shadow-lg transition-transform hover:-translate-y-0.5"
              >
                <svg viewBox="0 0 24 24" className="h-5 w-5 fill-current" aria-hidden><path d="M17.5 14.4c-.3-.1-1.7-.9-2-1-.3-.1-.5-.1-.7.1-.2.3-.7 1-.9 1.2-.2.2-.3.2-.6.1-.3-.1-1.2-.4-2.3-1.4-.9-.8-1.4-1.7-1.6-2-.2-.3 0-.5.1-.6.1-.1.3-.3.4-.5.1-.2.2-.3.3-.5 0-.2 0-.4-.1-.5-.1-.1-.7-1.6-.9-2.2-.2-.6-.5-.5-.7-.5h-.6c-.2 0-.5.1-.8.4-.3.3-1 1-1 2.4 0 1.4 1 2.8 1.2 3 .1.2 2 3.1 4.9 4.3.7.3 1.2.5 1.6.6.7.2 1.3.2 1.8.1.6-.1 1.7-.7 2-1.4.2-.7.2-1.2.2-1.4-.1-.1-.3-.2-.5-.2zM12 2C6.5 2 2 6.5 2 12c0 1.8.5 3.5 1.3 4.9L2 22l5.3-1.4c1.4.8 3 1.2 4.7 1.2 5.5 0 10-4.5 10-10S17.5 2 12 2z"/></svg>
...
function Row({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-3 border-b border-navy-foreground/10 py-1.5 last:border-0">
      <span className="text-xs uppercase tracking-wider text-navy-foreground/60">{label}</span>
      <span className={`text-right font-medium text-navy-foreground ${mono ? "font-mono text-xs" : ""}`}>{value}</span>
    </div>
  );
}
